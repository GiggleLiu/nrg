...
def get_expect_RDM(scale,dm,which,smearing_method='log_gaussian',b=0.4,diag=False):
    ...
    #get A(w),B(w)
    wlist=[];Alist=[];Blist=[]
    for i in iterator:  #note that the i here stands for hamiltonian iteration(site index + 1).
        t0=time.time()
        ...
        delta_peak=smethod(x=ws[...,newaxis,newaxis],mean=Ei[:,newaxis]-Ei[newaxis,:],b=b)
        ...
        wlist.append(ws);Alist.append(A);Blist.append(B)
        t1=time.time()
        print rho.shape,'(elapse: %s)'%(t1-t0)
    wlist=concatenate(wlist);Alist=concatenate(Alist,axis=0);Blist=concatenate(Blist,axis=0)
...
class ARGobj(object):
    ...
    def measure(self,scale,tdmanager,**kwargs):
        ...
        '''
        mval=array([self.get_expect(scale,tdmanager.tls[i],**kwargs) for i in xrange(scale.nz)])
        self.data=mval.mean(axis=0)

    def show(self):
        '''show data'''
        if self.data is None:
            warnings.warn('No Data is available! You need to measure this observable before showing.')
...
class ARGobj_A(ARGobj):
    ...
    def get_expect(self,scale,dm,diag=False,**kwargs):
        ...
        **kwargs:
        ...
        if isinstance(dm,FDMManager):
            fds=['f%s%sd'%(self.site,'up' if spin==0 else 'dn') for spin in self.spinindex]
            wlist,Alist=get_expect_FDM(scale,dm,opspace=(fds,fds,'TF'),diag=diag,**kwargs)
            return wlist,Alist
        elif isinstance(dm,RDMManager):
            ms=get_expect_RDM(scale,dm,diag=diag,*args,**kwargs)
            return ms
        else:
            raise Exception('parameter dm is not qualified @get_expect! should be a desity matrix NRG manager!')
        return ms
    ...
    def show(self,spin,*args):
        ...
        if spin[0]!=spin[1]:
            ...
            legend=[r'Im[A_{%s%s}]'%(spin[0],spin[1]),r'Im[A_{%s%s}]'%(spin[0],spin[1])]
        else:
...
class ARGobj_Sigma(ARGobj):
    ...
    def show(self,viewpoint,wlist=None,interp='spline',*args):
        ...
        plts=[]
        nband=self.nband
        if wlist is None: wlist=self.data[0]
        ...
        legends=[r'Im[$\Sigma_{%s%s}$]'%(i,j),r'Im$[G_{%s%s}(\omega)]$'%(i,j),r'Im$[F_{%s%s}(\omega)]$'%(i,j)]
        if i!=j:
            plts+=plot(wlist,Glist[:,i,j].real,lw=1,ls='--')
            plts+=plot(wlist,Flist[:,i,j].real,lw=1,ls='--')
            legends+=[r'Re$[G_{%s%s}(\omega)]$'%(i,j),r'Re$[F_{%s%s}(\omega)]$'%(i,j)]
    ...
    def get_expect(self,scale,dm,wspan,nw,diag=False,smearing_method='gaussian',b=1.,w0=None,b0=None,**kwargs):
        ...
        **kwargs:
        ...
        if isinstance(dm,FDMManager):
            ...
            fffs=['fff%s%s'%(self.site,'up'),'fff%s%s'%(self.site,'dn')]
            binner_A=get_expect_FDM(scale,dm,opspace=(fds,fds,'TF'),diag=diag,binner=deepcopy(binner),**kwargs)
            binner_B=get_expect_FDM(scale,dm,opspace=(fffs,fds,'FF'),diag=diag,binner=deepcopy(binner),**kwargs)
            wlist=binner.bins
            nband=len(fds)
            ...
            for i in xrange(nband):
                for j in xrange(nband):
                    Alist[:,i,j]=binner_A[i,j].get_spec(wlist=wlist,smearing_method=smearing_method,b=b*50,b0=b0)
                    Blist[:,i,j]=binner_B[i,j].get_spec(wlist=wlist,smearing_method=smearing_method,b=b*50,b0=b0)

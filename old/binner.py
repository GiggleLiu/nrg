...
class Binner(object):
    ...
    def __init__(self, bins, weights=None):
        ...
        if weights is None:
            weights = zeros(self.nbin, dtype=complex128)
    ...
    def get_spec(self, wlist, smearing_method='gaussian', window=(-Inf, Inf), b=1.):
        ...
        if smearing_method == 'gaussian':
            b = bandwidth / N * b
            alist = array([gaussian(x=w, mean=el, weights=wl, b=b).sum()
                           for w in wlist])
        elif smearing_method == 'log-gaussian':
            b = 15. * bandwidth / N * b
            alist = log_gaussian_fast(
                x=wlist, mean=el, weights=wl, b=b * ones(len(wlist)))
        elif smearing_method == 'lorenzian':
            pmask = el > 0
            obins = concatenate([el[~pmask], [0], el[pmask]])
            b = diff(obins) * 1.5 * b
            alist = array([lorenzian(x=w, mean=el, weights=wl, b=b).sum()
...
def get_binner(D, N, scale_type='log', w0=1e-6):
    ...
    if tp == 'linear':
        ...
        bins = concatenate(
            [-bins_neg[::-1], -bins_neg_s[::-1], bins_pos_s, bins_pos])
        return Binner(bins)
    else:

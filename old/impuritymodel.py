...
from tba.hgen import H2G, op_from_mats, op_on_bond, op_U
from tba.lattice import Bond
from nrgmap import construct_tridmat, Chain
from pymps.construct.opstringlib import op2collection, insert_Zs
...
def scale_bath(bath, Lambda):
    ...
    '''
    if ndim(Lambda) == 0:
        Lambda = Lambda**arange(bath.nsite)
...
class ImpurityModel(object):
    ...
    def get_H0(self):
        ...
        if self.impurity.H0 is not None:
            elist = concatenate(
                [self.impurity.H0[newaxis, ...], elist], axis=0)
    ...
    def get_opc(self):
        ...
        opc = op_on_bond('E', spaceconfig=spaceconfig,
                         mats=elist, bonds=ebonds)
        opc = opc + op_on_bond('T', spaceconfig=spaceconfig,
                               mats=concatenate([tlist2, tlist], axis=0), bonds=tbonds2 + tbonds)
        opc.label = 'H'

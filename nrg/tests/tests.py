...
class SChainTest(object):
    ...
    def __init__(self, which='flat'):
        ...
        wlist = get_wlist(w0=1e-12, Nw=10000, mesh_type='log',
        ...
        expander = RGHGen(
            H=self.baths[0], spaceconfig=spaceconfig, evolutor_type='null')

    def set_rhofunc(self, which):
        if which == 'flat':
            U = 1e-3
            Gamma = U / 10 / pi
            self.rhofunc = lambda w: zeros([2, 2]) if abs(
    ...
    def test_chainopc(self):
        ...
        for bath, scale in zip(self.baths, self.scales):
            scaledchain = ScaledChain(
                impurity=self.impurity, bath=bath, scale=scale)
    ...
    def test_nrg(self):
        ...
        for bath, scale in zip(self.baths, self.scales):
            scaledchain = ScaledChain(
                impurity=self.impurity, bath=bath, scale=scale)
            pdb.set_trace()
            scaledchain_ghost = ScaledChain(
                impurity=NullImp(), bath=bath, scale=scale)
            evolutor_ghost, bms_ghost, elist_ghost = NRGSolve(
                scaledchain_ghost, good_number=good_number)
            evolutor, bms, elist = NRGSolve(
                scaledchain, good_number=good_number)
            Tchi = RGobj_Tchi(spaceconfig=self.impurity.spaceconfig, mode='s')
            tchiflows.append(Tchi.measure(
                bms, elist=elist, scaling_factors=scaledchain.scaling_factors, M_axis=good_number.index('M')))
            tchiflows_ghost.append(Tchi.measure(
                bms_ghost, elist=elist_ghost, scaling_factors=scaledchain_ghost.scaling_factors, M_axis=good_number.index('M')))
...
class TestBinner(object):
    ...
    def test_gaussian(self):
        ...
        pl = 1. / sqrt(2 * pi * sigma**2) * \
            exp(-(wlist - mu)**2 / 2. / sigma**2)
        ion()
        smethods = ['lorenzian', 'gaussian']
        for sm in smethods:
            spec = binner.get_spec(
                wlist=wlist, smearing_method=sm, b=2., window=[-1, 1]).real / nsample
            plot(wlist, spec)
        plot(wlist, pl)

...
class ScaleTest(object):
    ...
    def test_interval(self):
        ...
        for i in range(self.N):
            z = self.scale.z
            assert_allclose(self.scale.interval(i + 1), self.D * array(
                [self.Lambda**-(i + z), self.Lambda**(0 if i == 0 else (-i + 1 - z))]))
            assert_allclose(self.scale.interval(-i - 1), -
                            self.scale.interval(i + 1)[::-1])

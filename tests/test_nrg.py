...
from tba.hgen.utils import sz,sx,sy,quicksave,quickload
from nrg import *
from lib import *
...
class NRGTest(object):
    ...
    def get_imp_bath(self,which='flat'):
        ...
        wlist=get_wlist(w0=1e-15,Nw=10000,mesh_type='log',Gap=0.1 if which=='sc' else 0,D=1.)
    ...
    def get_instance(self,which):
        if which=='flat':
            ...
            rhofunc=lambda w:zeros([2,2]) if abs(w)>1. else (Gamma/pi*identity(2))
    ...
    def test_nrg(self):
        ...
        #empty run
        res=NRGSolve(impurity=NullImp(),Lambda=sqrt(Lambda),bath=bath,bmg=bmg,maxN=500)
        ...
        #loaded run
        res=NRGSolve(imp,bath,bmg=bmg,Lambda=sqrt(Lambda),maxN=500,call_back=None)
    ...
    def test_tchi(self):
        ...
        Sz2s=Tscale_qnumber(mps=mps,EL=EL,TL=TL,qfunc=lambda x:x**2)[1:,1]/4.-\
                Tscale_qnumber(mps=mps0,EL=EL0,TL=TL[1:],qfunc=lambda x:x**2)[:,1]/4.
        ion()
